...
description: Connect external MCP tools or expose NoteGen to another AI client through a local MCP service.
icon: Puzzle
---
...
<NoteGenDocPreview kind="settings" section="mcp" lang="en" label="MCP settings" />
...
## Let another client access NoteGen

Desktop MCP settings include an Access NoteGen tab. When enabled, NoteGen runs a local Streamable HTTP MCP service on `127.0.0.1`, allowing Claude Desktop, Codex, or another compatible client to work with the current workspace.

1. Open Access NoteGen and enable the service.
2. Keep the default port or select an unused port from 1024–65535.
3. Select Connect/Copy configuration. NoteGen generates JSON containing the local URL and a Bearer token.
4. Add the configuration to the other client and reload its MCP connections.

The service listens only on the local machine and uses port `37422` by default. Its token is a credential for the displayed workspace. Never paste it into a public conversation or issue. If exposure is possible, reset the access token and update every client.

After switching workspaces, verify the new scope shown in NoteGen before allowing an external client to continue write or delete operations.
...
- External Servers lets NoteGen call someone else's tools. Access NoteGen reverses that direction; treat their permission boundaries separately.
