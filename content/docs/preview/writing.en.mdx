...
description: Edit Markdown visually or as source, and organize work with tabs, split groups, and separate windows.
icon: PenTool
---

NoteGen supports a Tiptap 3 visual editor and a source mode that preserves raw Markdown. Multiple files can be open in tabs, split groups, or separate desktop windows.

<NoteGenDocPreview kind="desktop-writing" lang="en" label="Desktop Markdown writing interface" />
...
- Tabs can be split into editor groups to view different files side by side. Supported text files can also open in a separate desktop window.
- A customizable new-tab page can show recent files and shortcut modules.
...
Visual mode renders headings, tables, images, and other Markdown structures directly. Source mode is useful for exact characters, frontmatter, or unsupported syntax. Choose the default mode in Editor Settings and optionally enable source line numbers and wrapping.

Both modes edit the same file. After switching for the first time, verify complex HTML, custom syntax, and very large documents.
...
- Drag a heading in the outline to move that heading and its section content together.
- View and customize all editor shortcuts under Settings → Shortcuts; conflicts with global shortcuts are detected.
- The status area can show characters, words, lines, and editor working state.
...
<NoteGenDocPreview kind="mobile" screen="writing" lang="en" label="Mobile Markdown writing interface" />
