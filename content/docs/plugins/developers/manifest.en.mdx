...
| `entry` | For scripted plugins | Safe package-relative JavaScript path no longer than 240 UTF-8 bytes and ending in `.js` |
...
| `resources` | No | Theme, application language, file icon, and document preview declarations |
...
- `apiVersion` is the SemVer compatibility range accepted by the plugin and must currently include the host's concrete API (`0.1.4` in SDK 0.1.6). After activation, `ctx.plugin.apiVersion` is that concrete host version, not this range string.
...
A non-empty resource-only package with no runtime contributions or activation events may omit `entry`; see [resource extensions](/en/docs/plugins/developers/resources). When present, `entry` must be a safe relative path no longer than 240 UTF-8 bytes to a UTF-8 `.js` file no larger than 5 MiB and containing no NUL byte.
...
- `views`, which add host-rendered `left-sidebar`, `right-sidebar`, `editor-tab`, `settings`, or [title bar](/en/docs/plugins/developers/title-bar) entries.
