...
| --- | --- | --- |
| npm package version | `@notegen/plugin-api@0.1.5` | Selects a TypeScript package release |
| `PLUGIN_API_VERSION` | `0.1.3` | Identifies the concrete host protocol described by that package |
...
| `ctx.plugin.apiVersion` | `0.1.3` | Reports the concrete API version implemented by the running host |

The protocol is still in the `0.x` series. Declare the minimum required by the capabilities you use: a baseline range may be `^0.1.0`, while title bar components require at least `>=0.1.3`. A package fix does not necessarily change the protocol; check the exported value for each release. Do not substitute `minAppVersion` for `apiVersion`, or confuse the manifest range with the runtime's concrete version.
...
SDK `0.1.5` is published on npm and describes protocol `0.1.3`. Resource packs require `>=0.1.2`; title bar views require `>=0.1.3`. Keep `minAppVersion` tied to a host version you have actually verified.

See [resource extensions](/en/docs/plugins/developers/resources) and [title bar components](/en/docs/plugins/developers/title-bar).
