...
`PluginContext` contains the `log`, `commands`, `workspace`, `calendar`, `fileIcons`, `notes`, `attachments`, `editor`, `storage`, `ui`, `network`, `i18n`, and `settings` namespaces, together with the lifecycle `signal`. This page documents their behavior; use the package's `PluginContext` and related exports as the complete signature source.

`ctx.plugin.id` and `ctx.plugin.version` come from the current plugin manifest. `ctx.plugin.apiVersion` is the concrete host API version, `"0.1.3"` in SDK 0.1.5; a manifest may keep the compatible range `"apiVersion": "^0.1.0"`.
...
The ordinary plugin context does not provide AI, secret storage, binary network responses, raw filesystem access, SQL, processes, Tauri commands, custom HTML/WebViews, or native capabilities. Attachments use a separate restricted Base64 API, not raw filesystem access. Declaring an unknown permission makes installation fail.
...
## Icon rules and preview access

`ctx.fileIcons.setRules(rules)` replaces runtime rules; `clear()` restores static rules. Neither grants file access. Preview scripts use a private MessagePort to read the displayed file under an `attachments.read` grant, without host DOM access or arbitrary Tauri calls. See [resource extensions](/en/docs/plugins/developers/resources).
