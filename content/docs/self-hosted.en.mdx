...
The official image is `ghcr.io/codexu/note-gen-server` and supports `linux/amd64` and `linux/arm64`. The current experimental build uses the `edge` tag, so the server does not need Node.js, pnpm, or a local source build.
...
| `NOTEGEN_SERVER_IMAGE` | Defaults to experimental `edge`; use a `sha-<commit>` tag to reproduce a specific build |
...
The `edge` image follows active development and may include configuration or database changes. Before upgrading, review repository notes and create a consistent backup of PostgreSQL and attachment data.
...
To pin the current experimental build, replace `edge` in `NOTEGEN_SERVER_IMAGE` with its `sha-<commit>` tag. Version tags can be used after a formal release is published.
